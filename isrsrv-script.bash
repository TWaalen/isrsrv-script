...
script_send_crash_email() {
	if [[ "$EMAIL_CRASH" == "1" ]]; then
		...
		ONLY SEND game_logs.zip TO THE DEVS IF NEED BE! DON'T SEND OTHER ARCHIVES!
...
case "$1" in
	-help)
		...
		echo -e "${GREEN}ssk_check_email ${RED}- ${GREEN}Checks the SSK's creation/modification date and sends out and if nearing expiration and if the email setting is enabled.${NC}"
	...
	*)
	...
	echo ""
	echo "Usage: $0 {start|stop|restart|save|sync|backup|autobackup|deloldbackup|delete_save|change_branch|ssk_check|ssk_check_email|reinstall_services|reinstall_prefix|update|status|install \"server command\"}"
	exit 1
	;;
esac
